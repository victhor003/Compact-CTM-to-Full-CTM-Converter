...
class MainApplication(tk.Frame):
    def __init__(self, main):
        ...
        # Frames
        self.input_frame = tk.LabelFrame(main,
                                         text="Input",
                                         labelanchor="n"
                                         )
        self.output_frame = tk.LabelFrame(main,
                                          text="Output",
                                          labelanchor="n"
                                          )
        self.side_frame = tk.Frame(main,
                                   width=90,
                                   height=500
                                   )
        self.input_img_frame = tk.LabelFrame(self.input_frame,
                                             text="Input Images",
                                             width=320, height=64,
                                             relief="sunken"
                                             )
        ...
        self.res_combo = tk.ttk.Combobox(self.side_frame, value=res_options, state="readonly")
        ...
        self.preview_button.grid(row=2, column=0)
        self.comfirm_conversion_button.grid(row=2, column=1)

        # Draw Frames
        self.input_frame.grid(row=0, column=0, pady=5, padx=5)
        self.output_frame.grid(row=1, column=0, pady=5, padx=5)
        self.side_frame.grid(row=0, column=1, rowspan=2, pady=5, padx=5)
    ...
    def searchfolder(self, mode):
        ...
        main.selectedfolder = tk.filedialog.askdirectory(title='Select ' + self.mode[mode] + ' Folder', initialdir="/")
        input_equals_output = False
        if os.path.exists(main.selectedfolder):
            if mode == 0:
                self.img_l.clear_loaded_files()
                self.input_entry.config(state='normal')
                self.input_entry.delete(0, tk.END)
                self.input_entry.insert(0, main.selectedfolder)
                self.input_entry.config(state='readonly')
                self.img_l.load_imgs(main.selectedfolder)
            ...
            if mode == 1:
                if os.access(main.selectedfolder, os.W_OK):
                    self.output_entry.config(state='normal')
                    self.output_entry.delete(0, tk.END)
                    self.output_entry.insert(0, main.selectedfolder)
    ...
    def preview_output(self):
        self.preview_button.config(state='disabled')
        cropsize = self.res_cropping[self.res_combo.get()]
...
def main():
    ...
    root.title('Compact CTM to Full CTM Converter - by victhor003')
