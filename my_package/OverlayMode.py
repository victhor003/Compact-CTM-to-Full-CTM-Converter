...
class OverlayTab(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        # Frames
        self.input_frame = tk.LabelFrame(self, text="Compact Overlay Input", labelanchor="n")
        self.output_frame = tk.LabelFrame(self, text="Full Overlay Output", labelanchor="n")
        self.side_frame = tk.Frame(self, width=90, height=500)
        self.input_img_frame = tk.LabelFrame(self.input_frame, text="Input Images", width=320, height=64,
                                             relief="sunken")
        ...
        # Imput Images Frame
        self.ph_imgage_1 = ImageTk.PhotoImage(Image.new('RGBA', (512, 128), color='#F0F0F0'))
        ...
        self.ph_imgage_2 = ImageTk.PhotoImage(Image.new('RGBA', (448, 192), color='#F0F0F0'))
    ...
    def searchfolder(self, mode):
        ...
        self.selectedfolder = tk.filedialog.askdirectory(title='Select ' + self.mode[mode] + ' Folder',
                                                              initialdir="/")
        input_equals_output = False
        if os.path.exists(self.selectedfolder):
            if mode == 0:
                self.img_l.clear_loaded_files()
                self.input_entry.config(state='normal')
                self.input_entry.delete(0, tk.END)
                self.input_entry.insert(0, self.selectedfolder)
                self.input_entry.config(state='readonly')
                self.img_l.load_imgs(self.selectedfolder, ["0", "1", "2", "3"])
            ...
            if mode == 1:
                if os.access(self.selectedfolder, os.W_OK):
                    self.output_entry.config(state='normal')
                    self.output_entry.delete(0, tk.END)
                    self.output_entry.insert(0, self.selectedfolder)
    ...
    def preview_output(self):
        ...
        self.output = self.img_l.generate_overlay(cropsize)
        self.output_preview = self.output
        print(self.output_preview)
        self.composed_output_preview = ic.to_grid(self.output_preview, 7)
        self.composed_output_preview = self.composed_output_preview.resize((448, 192), Image.NEAREST)
